...
| 语言与代码质量     | 9   | effect、rust-skills、ruby-on-rails-best-practices、vercel-react-best-practices、sql-optimization、e2e-testing-patterns、owasp-security-check、modular-design-principles、skill-writing-best-practices                                | 各语言/领域的深度守则，见 [[#九、语言、安全与逆向]]                                                       |
| 逆向工程        | 3   | protocol-reverse-engineering、reverse-engineering-android-malware-with-jadx、reverse-engineering-tools                                                                                                                         | 网络协议 / APK / 驱动反作弊，见 [[#九、语言、安全与逆向]]；android-reverse-engineering 已修复待重启验证，见 [[#十二、加载机制]] |
...
最后一批：**语言守则**（Effect / Rust / Rails）、**质量与安全**（SQL、E2E、OWASP、模块化）、**逆向工程**（三件）。

### 语言守则

**effect**：本仓库专属——这份代码库用 Effect 做**类型化、可组合的 TypeScript 服务、schema 与工作流**（Effect v4 / effect-smol）。内含 Source of Truth 指针、编码守则与测试模式。在本仓库写 Effect 代码时自动参考。

**rust-skills**：**265 条规则 26 类**——所有权、错误处理、async、并发、unsafe、API 设计、内存优化、性能、数值安全、类型转换、serde、模式匹配、宏、闭包、可观测性、测试与常见反模式。`/rust-skills` 调用。

**ruby-on-rails-best-practices**：Basecamp 系惯例——目录结构、models / controllers / jobs / concerns 组织；concerns 命名有明确约定：行为用 `-able` 后缀（searchable）、特性用名词，且区分共享 concerns 与 model 专属 concerns。

### 质量与安全

**sql-optimization**：跨数据库（MySQL / PostgreSQL / SQL Server / Oracle 等）的查询调优——核心优化领域、查询反模式、索引管理、性能监控查询、通用优化清单与方法论。慢查询、索引设计直接点名它。

**e2e-testing-patterns**：Playwright / Cypress 的端到端测试模式库——核心概念、成套模式与实例、最佳实践、常见坑、**失败测试的调试**。与 frontend-testing-best-practices 分工：那件讲理念（E2E 优先、少 mock），这件讲具体怎么写、怎么救。

**owasp-security-check**：基于 OWASP Top 10 与 Web 安全惯例的审计——审计工作流、规则汇总、常见漏洞模式、**严重度速查表**。触发场景：查代码漏洞、评审认证授权（auth/authz）、交付前 API 安全审计。

**modular-design-principles**：技术无关的模块化设计评审——分层心智模型、**十条原则**、典型违规、创建 bounded context 的工作流、拆分 / 合并判断、context 内子单元、架构合规检查、提案前检查清单。「该不该拆这个包」类问题找它。

> 注意：其描述引用的 `modular-decomposition`、`decomposition-planning-roadmap`（完整拆解管线）本库未安装——它只做评审与原则，不做整仓拆解执行。

### 逆向工程

- **protocol-reverse-engineering**：网络协议逆向——流量捕获（指定接口、过滤器、落盘、环形轮转）、协议剖析、自定义协议文档化。
- **reverse-engineering-android-malware-with-jadx**：用 JADX 反编译恶意 APK——工作流：APK 基本信息 → 提取 AndroidManifest → androguard 权限分析 → 危险权限识别。安全分析向。
- **reverse-engineering-tools**：受保护游戏与反作弊组件——覆盖用户态 / 内核态 / hypervisor 感知环境；调试器、反汇编反编译、内存分析、DBI 动态插桩、反分析绕过；驱动、IOCTL 协议、回调分析。
- **android-reverse-engineering**（已修复待重启验证，见 [[#十二、加载机制]]）：APK / XAPK / JAR / AAR 解包，jadx + Fernflower 双引擎可对照，提取 Retrofit / OkHttp / Volley API 端点，UI → 网络层调用链追踪。开发向，与上面那件恶意样本分析分工不同。
...
- **2026-09-09（7）**：新增「九、语言、安全与逆向」章节收尾——语言守则（effect 仓库专属 / rust 265 条 26 类 / Rails concerns 命名约定）、质量与安全（sql-optimization、e2e 与 frontend-testing 的理念/模式分工、OWASP 审计、modular-design 十条原则）、逆向三件 + 已修复的 android-reverse-engineering。记录 modular-design-principles 引用的 `modular-decomposition`、`decomposition-planning-roadmap` 未安装。**至此全库 175 技能均有说明**；章节顺延（易混→十、演练→十一、加载→十二、速查→十三、日志→十四）。

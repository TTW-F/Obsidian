...
| 品牌语调 | `/blog brand init` | 生成 BRAND.md / VOICE.md，所有子技能自动加载 |
| 读者画像、内容分发 | `/blog persona`、`/blog repurpose` | 长文改写为其他平台格式 |
...
主库里有两个技能当前会话未暴露，原因已查实（2026-09-09）：

- **`android-reverse-engineering`**（APK 解包提取 API）：frontmatter 不符合 Agent Skills 规范——缺 `name` 字段，且用了规范外的 `trigger:` 字段。**修复**：补上 `name: android-reverse-engineering`、删除 `trigger` 行，重启会话验证。
- **`claude-blog-brain`**（Obsidian 博客内容中枢）：description 长达 1709 字符，超过规范的 1024 字符上限，疑似因此被拒。**修复**：把描述压缩到 1024 字符以内。

修好后用 `/find-skills` 或查会话技能列表确认是否恢复。
...
- **2026-09-09**：查实两个未加载技能的原因并写明修复方法（缺 `name` 字段 / description 超 1024 上限）；工程索引表恢复到主流程各节的跳转链接；补 blog 品牌语调与内容分发入口。

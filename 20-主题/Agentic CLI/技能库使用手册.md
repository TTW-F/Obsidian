...
先澄清一个关键事实：**每个 agent 只加载自己目录下的技能**。本机技能分散在 **9+ 个 agent 生态**中，去重后 **100+ 个已安装用户技能**（不含插件市场的数百个备选）：
...
- **Matt Pocock 工程系列**是主心骨：`.codex/skills`、`.workbuddy/skills/codex`、`.claude/skills` 各有一份完整版（核心 16 个），其余生态只散布部分（grilling、to-spec、wayfinder 等）。
...
| 生态 | 技能目录 | 数量 | 主要内容 |
| --- | --- | --- | --- |
| **Codex CLI** | `C:\Users\TTW\.codex\skills` | 30 | Matt 核心 16 个 + impeccable、taste-skill + 通用 6 件套 + **逆向工程 4 个** + 系统 2 个目录（imagegen、skill-installer 等 6 个内置） |

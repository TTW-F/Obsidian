...
flowchart TD
  subgraph "本机技能全景"
    ...
    B["Claude Code<br/>.claude/skills (12)"]
    C["opencode<br/>.config/opencode (6)"]
...
| **Claude Code** | `C:\Users\TTW\.claude\skills` | 12 | 通用 6 件套 + design-taste-frontend、impeccable、grilling、domain-modeling、codebase-design、improve-codebase-architecture |
...
> **为什么 opencode 里只看到 15 个**：opencode 只聚合 `.agents`、`.claude`、`.config/opencode` 三处（即当前对话可用的 15 个）；`.codex`、`.gemini`、`.workbuddy` 等目录的技能要打开对应工具才生效。
...
- **Matt Pocock 系列**：`.codex/skills` 与 `.workbuddy/skills/codex` 各一份完整版；`.agents`/`.hermes`/`.qoder`/`.trae`/`.kiro` 只有 to-spec、wayfinder 等散件；`.claude` 只有 grilling、domain-modeling 等 6 个。

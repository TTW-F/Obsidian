...
| 站点体检、外链、竞对 | `/seo doctor`、`/seo backlinks`、`/seo competitor-pages` | `/seo setup` 做初始配置 |
...
| 广告策略/预算/创意 | `/ads-plan`、`/ads-budget`、`/ads-creative`、`/ads-launch` | 子技能两组命名：平台类 ads-{google,meta,…}、工作流类 ads-{plan,budget,…}，均可单独调用 |
...
- **2026-09-09**：查实并**修复**两个未加载技能——`android-reverse-engineering` 补 `name`、删规范外 `trigger`；`claude-blog-brain` description 1709 → 742 字符（待重启验证加载）。工程索引表恢复主流程跳转链接；补 blog 品牌语调与内容分发入口、`/seo doctor|backlinks|competitor-pages` 与 ads 工作流子技能。
